    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-[480px]">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 pt-2">
          ...
          {/* Department — smart combo: select existing OR type new */}
          <Controller
            name="department"
            control={control}
            render={() => (
              <div className="space-y-2">
                ...
                <div className="relative" ref={dropdownRef}>
                  ...
                  {showDropdown && (
                    <div className="absolute z-50 mt-1 w-full rounded-md border border-gray-200 bg-white shadow-lg max-h-52 overflow-y-auto">
                      ...
                      {/* Option to create brand-new department */}
                      {deptInput &&
                        !departments.some(
                      ...
                      {filteredDepts.length === 0 && !deptInput && (
                        <p className="px-3 py-2 text-xs text-gray-400">
                          Start typing to create a new department
                        </p>
                      )}
                    </div>
                  )}
                ...
                {deptInput &&
                  !departments.some(
                    ...
                    <p className="text-[0.75rem] text-amber-600 flex items-center gap-1">
                      <Plus className="h-3 w-3" />
                      New department will be created with default full permissions
                    </p>
                  )}
              </div>
            )}
