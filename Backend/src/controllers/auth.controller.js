...
const login = async (req, res, next) => {
    try {
        ...
        const user = await User.findOne({ email: email.toLowerCase() })
            .select('+password')
            .populate('company', 'name email logo currency invoicePrefix isActive plan industryType');
...
const getMe = async (req, res, next) => {
    try {
        const user = await User.findById(req.user._id)
            .select('-password')
            .populate('company', 'name email logo currency invoicePrefix plan planExpiry isActive industryType');
